---
name: archgate
description: Оценка архитектурного решения по 7 характеристикам ЭМОГССБ (v3.1 — фильтр допуска, атрибут-сценарии, совет затронутых сторон; профиль без агрегатного балла, conjunctive screening). Используй когда пользователь предлагает архитектурное решение, новый инструмент или системное изменение.
...
> **СТОП. Не переходи к шагу 2 без ответа пользователя на ВСЕ ТРИ вопроса.**
...
- Для системных решений дефолт — второе мнение напарника (peer-сессия; подключается расширением `archgate.checks`).
- Собранный совет фиксируется в DRR, секция «Совет затронутых сторон»: кого спросили → какой совет дали → что принято/отвергнуто и почему. Ответ «никого не затрагивает» допустим — с обоснованием, оно тоже идёт в DRR.
...
- Если `core_result = fast_tracked`: облегчённый вердикт быстрого трека (абзац выше).
