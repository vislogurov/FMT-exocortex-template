...
> Записывает итоги закрывающейся недели в отдельный файл WeekReport + создаёт пост для клуба. Служит входом для session-prep (Пн 4:00).
> **Split (WP-297 ОПТ-5, issue #596):** факты недели живут в `WeekReport W{N} YYYY-MM-DD.md`, WeekPlan — только намерения. Раньше это место писало факты прямо в секцию WeekPlan («deprecated 2026-03-25» ошибочно утверждало, что отдельный файл не нужен вовсе — решение WP-297 это отменило, три файла роли стратега просто не подхватили).
...
4. Запиши контент-план в секцию «Итоги W{N}» в WeekReport (шаг 5)
...
### 5. Сохранение итогов в WeekReport

> **WeekPlan итогов не содержит** (WP-297 ОПТ-5) — только намерения следующей недели. Факты закрывающейся недели идут в отдельный файл.

1. Определи `{N}` и дату недели из имени закрывающегося `{{GOVERNANCE_REPO}}/current/WeekPlan W{N} YYYY-MM-DD.md` (та же неделя, тот же `{N}` и та же дата — файлы WeekPlan/WeekReport одной недели парные).
2. Создай (или, если уже существует — допиши) `{{GOVERNANCE_REPO}}/current/WeekReport W{N} YYYY-MM-DD.md` с frontmatter `type: week-report`, `week: W{N}`, `created: YYYY-MM-DD`.
3. Запиши туда секцию `## Итоги W{N}`: метрики, таблицу по репо, статусы РП, инсайты, carry-over, контент-план (формат — см. шаблон ниже).
...
1. Переключись на **роль Автора (R4)** и на основе секции «Итоги W{N}» в WeekReport (шаг 5) сформируй пост для клуба.
   ...
   **Обязательные данные от Стратега → Автору:**
   ...
   - Фокус следующей недели (из WeekPlan следующей недели — намерения, не факты закрывающейся)
...
Результат:
- Секция «Итоги W{N}» в WeekReport — как вход для session-prep
...
**Живой файл (единственная цель записи для 8a-8c):** `~/.claude/projects/{{CLAUDE_PROJECT_SLUG}}/memory/MEMORY.md` (тот же путь, что `note-review.md` уже использует, доступен и по симлинку `{{WORKSPACE_DIR}}/memory/MEMORY.md`). `{{GOVERNANCE_REPO}}/exocortex/MEMORY.md` — это резервная копия для восстановления на новой машине, синхронизация в нём одностороняя (живой → exocortex), её трогать здесь не нужно и нельзя: правка, сделанная только в `exocortex/`, теряется при следующей синхронизации, а агент в следующей сессии продолжает читать живой файл с неделей, которая должна была смениться (issue #708).
